...
function updateTotal() {
  ...
  let totalAmount = 0.00; // Initialize as a number

  for (var i = 0; i < cartProducts.length; i++) {
    const productPriceText = cartProducts[i].getElementsByClassName("cart-product-price")[0].innerText;
    const productPrice = parseFloat(productPriceText.replace("R$", "").replace(",", ".")); // Convert to a number
    const productQuantity = parseFloat(cartProducts[i].getElementsByClassName("product-qtd-input")[0].value); // Convert to a number

    totalAmount += productPrice * productQuantity;
  }

  totalAmount = totalAmount.toFixed(2); // Format with two decimal places
